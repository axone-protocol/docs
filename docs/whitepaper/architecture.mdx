...
Any smart contract deployed on the OKP4 blockchain uses the logic module to evaluate sharing conditions definitions and user access queries written in Prolog. Prolog is a powerful declarative programming language to manage from simple restrictions to complex rules. This language is not natively operable on-chain; the logic module adds Prolog interpretation capability.

The logic module includes custom predicates, like blockchain state clauses and various data operation utilities, enabling advanced governance possibilities.

The logic module has a virtual file system built in. Users can store Prolog programs and add a reference to another Prolog program. It enhances reusability and composability.  
...
The OKP4 blockchain uses production-grade modules from Cosmos SDK. These generic modules facilitate the implementation of protocol governance, token operation management, validator punishment mechanisms, and authentication of accounts and transactions.
...
* Ontology: the knowledge graph modelling all the different resources (e. g. datasets, computation services, infrastructure, orchestration engines, …), their descriptive properties and relations between each other. It also includes access rules and execution instructions linked to resources. The ontology is a whole of textual objects, a way to store a semantic representation of the Dataverse.
* Governance: the policies ruling the definition and the validation of resource permissions.  
...
* Orchestration: the processes that check the access conditions compliance defined by entities. Orchestration relates to resource authorisation controls and revenue-sharing distribution.

<center><img src="/img/content/whitepaper/arch-3pillars.webp"></img></center>

### Objectarium: unstructured data storage

Prolog code defines governance rules, the “law”. These pieces of logic programming (usually from `.pl` files) are stored in raw with an `objectarium` smart contract, which acts as a file system.
...
There is no restriction on the structure to allow ample expressiveness. But the OKP4 “core”, the minimal ontology scheme, requires the specification of an explicit relationship between an external resource, its governance rules and service execution instructions.
...
The rule cannot be changed; what is stored onchain is immutable. The one who instantiates the governance rule is the administrator, the only one able to destroy it. When a law is deleted, the main Prolog code stored in `objectarium` is unpinned. All object with no other pin is deleted.
...
### Pactum: service agreement contract

This smart contract guarantees the orchestration, the respect of the law and the value allocations.

A `pactum` instantiation defines a service agreement between at least two parties, details the prerequisites that fulfill its enforcement and the actions to proceed accordingly. These conditions to execute contractual obligations are materialized as a set of terms previously stored with the `law-stone` smart contract.  

A `pactum` instance records service execution states and applies token revenue-sharing distribution if conditions are met. It can also flag invalid service execution and block escrow in consequence.
...
`zone-hub` also handles state reportings from orchestration executions. It redirects messages to `pactum` instances.
...
Service providers must add service execution instructions regarding secrets keys alongside the governance rules when referencing a resource to the on-chain ontology.  
...
Standards are not limited to those of the industry: they are specifications like the E-Government Metadata Standard (e-GMS). Thus the UK public sector has a metadata standard for making data handling consistent in order to promote efficient use of Web pages and documents.  
...
Two companies (Corp A and Corp B) grant access to their databases, while a third company (Corp C)  provides a machine-learning training workflow using the data. A data scientist (Individual D) wants to get new knowledge with resources from Corp A, Corp B and Corp C.
...
1. Each on their own, Corp A and Corp B indicate through an administration portal the underlying technology of their respective database. They store access tokens with a Secret Management Service, with authorization for services from Corp C.
...
3. Metadata indicates that resources shared by Corp A and Corp B are compatible for a ML job. Corp C creates a workflow training a model with data provided by Corp A and Corp B. Corp C also uses the administration portal to submit its training model algorithm, governance rules, and service execution instructions.

4. Individual D parameters its cloud environment with the portal, setting up how workflow engines should store query results.
...
6. The orchestration service running the ML Workflow listens the event from the blockchain. It executes the workflow from Corp C training algorithm, using Corp A & Corp B data.
...
8. The workflow engine stores the result in the provided Individual D’s storage solution.
...
OKP4-->>ML Workflow: Chain event triggering
ML Workflow->> Secret: Get resources access
Secret->> ML Workflow: Resources secret keys
loop Db A, Db B
ML Workflow->> Db A/B: Resource query
Db A/B->> ML Workflow: Resource result
end 
ML Workflow->> ML Workflow: Knowledge generation

Note right of ML Workflow: 7. 
ML Workflow->> OKP4: Workflow state report
OKP4->>OKP4: Service agreement exec ($KNOW rev share, ...)

Note right of ML Workflow: 8. 
ML Workflow->> Secret: Get cloud storage access keys
Secret->>ML Workflow: Cloud storage access keys
ML Workflow->> Storage: Store result
