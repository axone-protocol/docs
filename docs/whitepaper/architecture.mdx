...
It enforces the rules and policies that govern resource (datasets and services) authorization.
...
Two companies (Corp A and Corp B) grant access to their databases, while a third company (Corp C) provides a machine-learning training workflow using the data. A data scientist (Individual D) wants to get new knowledge and invokes an orchestration service that use resources from Corp A, Corp B and Corp C.
...
8. The orchestration service stores the result in the provided Individual D’s storage solution.
