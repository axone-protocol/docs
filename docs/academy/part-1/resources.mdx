...
Diving into the realm of off-chain resource sharing with Axone, it's crucial to understand the diverse landscape of digital resources that can be seamlessly shared. Digital resources, the building blocks for extracting insights and generating new knowledge, come in various forms and formats.
...
Digital Resources are the raw material from which insights and new knowledge are extracted. They encompass data and information presented in various forms (spreadsheets, text, videos, etc.) and expressed, carried, and stored in any format (.xml, .json, .txt, .mp4, etc.).
...
Understanding the intricate workings of the Axone protocol is essential, particularly concerning handling digital resources. Unlike traditional storage mechanisms, the Axone blockchain doesn't store these resources directly. Instead, providers maintain sovereignty over resource ownership.
...
**Example**: A Data provider shares a dataset. In the Axone Protocol, this dataset is uniquely identified by the DID `d0c63b88-44f8-4518-b62d-e141dd8fb624`.
