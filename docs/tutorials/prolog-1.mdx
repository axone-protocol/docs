...
Let's combine it all! To get back the Prolog code stored in the last `law-stone` (**`CODE_ID = 5`**, storage-oriented `objectarium` smart contract address = `okp41lppz4x9dtmccek2m6cezjlwwzup6pdqrkvxjpk95806c3dewgrfq602kgx`) instantiated by the okp4 address **`okp41z4ldttn975ku764er2nvtfq47n6n7jfxnkc2k9`**, you can type:
...
| jq '.answer.results[0].substitutions[0].term.name' | tr -d "'" | tail -c +2 | head -c -2 | tr -d "\\" | jq .
